...
def test_boolean_actions_offer_both_states():
    """A one-directional variant existed briefly for the induction's power toggle. The
    appliance turned out to accept no writes at all, so the toggle, its card and the
    generator policy behind it are gone — every boolean action offers both again."""
