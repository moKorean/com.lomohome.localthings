...
공식 Homey 앱스토어를 목표로 하는 경우, **사용자에게 개인키를 붙여넣게 하는 UI는 심사
리스크**입니다. 리프 전용 모델이 CA 개인키를 다루지 않는다는 점에서 유리하지만, 그래도 제출 전에
Athom에 문의하는 편이 안전합니다. 레퍼런스 통합이 HA 코어가 아니라 HACS로 배포하는 것도 같은
성격의 제약입니다.
