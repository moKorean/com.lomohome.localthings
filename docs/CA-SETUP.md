...
> **대부분은 이 문서가 필요하지 않습니다.** 0.7.0부터 앱이 첫 실행 때 클라이언트 인증서를
> 직접 발급합니다 — 설치하고 **기기 → 기기 추가**로 넘어가면 됩니다. 이 문서는 삼성
> `AC14K_M` CA가 서명한 인증서를 직접 넣고 싶은 경우를 위한 선택 안내입니다. 붙여넣은
> 인증서가 우선하며, 앱이 덮어쓰지 않습니다.
>
> 앱이 직접 발급할 수 있는 이유: 가전은 인증서에 담긴 식별자를 확인하고 서명한 주체는
> 확인하지 않습니다. 에어컨과 냉장고에서 실측했고, 즉석에서 만든 키로 서명한 인증서를 둘 다
> 수락했습니다. 삼성이 서명한 인증서를 보유하고 싶을 때, 또는 나중에 가전 업데이트로 앱이
> 발급한 인증서를 받지 않게 될 때 이 안내를 따르세요.

직접 넣고 싶다면: 컴퓨터에서 스크립트를 하나 돌려 파일 두 개를 만들고, 그 내용을 Homey 앱
설정에 붙여넣습니다. 가전마다 반복하지 않습니다.
