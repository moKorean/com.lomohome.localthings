...
**파이썬에서 발생하는 사용자 노출 문자열은 영어뿐입니다.** 즉 한국어 사용자가 다음 경우에 영어를 봅니다:

- 기기 사용 불가 이유 (`set_unavailable(str(exc))`)
- 쓰기 거부 토스트 — "The appliance rejected target_temperature=28"
- 페어링·복구의 검증 오류

Homey의 서버측 i18n(`i18n.translate`)으로 해결할 수 없습니다 — 8절과 같은 이유로 앱 언어가 `'en'`으로 해석되기 때문입니다(`locales/ko.json`이 로드되어 있어도). 고치려면 웹뷰가 알려준 UI 언어를 저장해 두고 파이썬 메시지에 직접 쓰는 방식이 필요합니다.
