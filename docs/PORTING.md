...
- `../localthings-reference/` — [mbillow/localthings](https://github.com/mbillow/localthings), `main` @ `0c2e219` (v0.19.0). 포팅 원본
- `../smartthings-local-reference/` — [QuiteYellow/SmartThings-Local](https://github.com/QuiteYellow/SmartThings-Local), `main` @ `280939d` (v0.1.2+12). 전송 계층 소스 + `setup_cert.py`. 앱은 PyPI의 `smartthings-local>=0.1.2`를 씁니다 — 0.1.2가 stateless DTLS 프로브(`dtls_probe`)를 추가했고 우리 포트 탐색이 그걸 씁니다
...
세 가지 고장이 재페어링 없이 복구됩니다 — 가전은 같은 가전이니까요.

| 증상 | repair 뷰가 하는 일 |
|---|---|
| 인증서 만료·손상 | 상태와 만료일을 보여주고, 앱 설정에서 교체하라고 안내 |
| 주소 변경 (자동 재배치 실패) | 시리얼로 서브넷을 훑어 찾아 기기를 재지정 |
| 일시적 무응답 | 저장된 주소로 연결을 확인 |

**주소를 직접 입력하는 경우에도 시리얼을 대조합니다.** 대조 없이 재지정하면 사용자가 실수로 다른 가전을 가리키게 만들 수 있고, 그건 unavailable보다 나쁩니다(9절과 같은 이유). 시리얼이 안 맞으면 적용을 거부하고 무엇이 발견됐는지 알려줍니다.
...
- **앱스토어 심사.** 계획 단계에서는 사용자에게 CA **개인키**를 붙여넣게 하는 앱이 공식 스토어 승인을 못 받을 것으로 보고 커뮤니티 스토어를 전제했습니다. **결과적으로는 통과했습니다** — 앱이 CA 개인키를 받지 않고 이미 발급된 리프 인증서만 저장하는 구조로 정리한 것이 유효했습니다. 심사에서 실제로 걸린 것은 인증서가 아니라 앱 이름과 스토어 이미지의 글자였고(v0.5.0에서 수정), 앱은 [공식 앱스토어에 등록](https://homey.app/a/com.lomohome.localthings/)되어 있습니다
...
이게 더 위험합니다. 같은 모델 두 대(검증 환경에는 에어컨 4대가 같은 보드)의 IP가 서로 바뀌면, 각 Homey 기기가 **다른 가전을 조용히 제어**합니다. 한 방의 에어컨을 끄면 다른 방이 꺼지는 식입니다. unavailable보다 나쁩니다.
...
```
삼성 에어컨   BWHYP3FL700219K   push, 22 subscriptions
삼성 에어컨   BWHYP3FL700231M   push, 22 subscriptions
삼성 인덕션   0NVH7DBL800355P   push, 4 subscriptions
```

(시리얼은 형식만 같게 바꾼 예시 값입니다 — 실제 기기를 식별하는 값이라 문서에 남기지 않습니다. 본인 기기의 값은 각 기기의 고급 설정에서 확인할 수 있습니다.)
...
**선언되지 않은 언어는 영어로 폴백됩니다** — 네 곳 전부 확인하고 `tests/test_i18n.py`로 고정했습니다.

| 표면 | 확인 방법 |
|---|---|
| 앱 매니페스트 (이름·설명·capability·플로우·설정 라벨) | 빌드된 `app.json`을 훑어 모든 i18n 객체에 `en` 키가 있는지 |
| 웹뷰 3개 (설정·페어링·복구) | `STRINGS.ko`에만 있는 키가 없는지 (있으면 다른 언어에서 `undefined`가 출력됨) |
| `locales/{en,ko}.json` | 키 집합 일치 |
| 레지스트리·서브 capability 이름 | `title('nl')`이 레지스트리 키가 아니라 영어를 반환하는지 |

테스트는 서식이 아니라 **불변식**을 검사합니다: `LANG`은 `'en'`으로 시작하고 `'ko'` 리터럴 외의 값으로 대입되지 않아야 합니다. `LANG = lang`처럼 해석된 값을 그대로 넣으면 테이블이 없는 언어 사용자에게 한국어나 깨진 값이 갑니다.

### 파이썬 메시지도 번역됩니다

**Homey의 서버측 i18n으로는 안 됩니다.** `i18n.get_language()`가 앱 언어를 반환하고 그 값이 `'en'`이라(8절), `locales/ko.json`이 로드되어 있어도 `translate()`는 영어를 돌려줍니다.

**웹뷰는 UI 언어를 압니다.** 그래서 웹뷰가 보고하고, 앱이 `ui_language` 설정에 저장하고, `lib/i18n.py`가 Homey와 같은 `locales/*.json`을 읽어 포맷합니다.

```
i18n.get_language  ->  "en"   (앱의 언어, 쓸 수 없음)
ui_language        ->  "ko"   (웹뷰가 보고한 값)
```

번역되는 메시지: 인증서 미등록, 인증서 저장 실패, IP 미입력, 종류 미인식, **다른 가전 감지**, 쓰기 불가, 값 미지원, 쓰기 거부, 복구 대상 불명.

설계상 유의점:

- **영어가 모든 단계의 폴백입니다.** 알 수 없는 언어·없는 키·플레이스홀더 불일치가 모두 폴백으로 처리되고 예외를 던지지 않습니다. 오류 메시지를 만드는 도중에 두 번째 실패를 내는 것이 최악입니다
- **기기는 언어를 기동 시 한 번 해석합니다.** 메시지는 폴링 루프와 capability 리스너에서 발생하는데, 실패마다 설정을 읽는 것은 낭비이고 하필 그 실패 중에 읽게 됩니다
- **capability id 대신 이름을 씁니다.** "localthings_ac_mode를 바꿀 수 없습니다"는 정확하지만 읽을 수 없습니다 — 서브 capability 제목이 이미 그 용도로 있습니다
- **업그레이드 직후에도 동작합니다.** `ui_language`가 비어 있으면 이전 페어링 세션이 남긴 `pair_env`의 `resolved` 값으로 초기화합니다. 그러지 않으면 어떤 화면이든 한 번 열기 전까지 영어가 나옵니다
