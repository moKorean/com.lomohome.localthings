...
### 알림 카운터는 채널 상태가 아니라 변화 횟수입니다 (실측 2026-08-03)

`_notified`는 **재구독 때만** 비워지고, 그 사이 도착한 **변화 알림**을 모읍니다. 등록 응답은
세지 않습니다 — 전송 라이브러리의 `subscribe` docstring은 "등록 응답이 곧 첫 알림"이라고
서술하지만 **가전은 그렇게 하지 않습니다.**

손님방 에어컨(0.6.7, 간격 0.05초 적용된 상태)에서:

```
전원 Off        →  0/27   silent_rounds=1  retry=1200
전원 On, 70초 후 →  7/27   silent_rounds=1  retry=1200   ← 재구독 없음
```

`silent_rounds`와 `retry`가 움직이지 않았으므로 27개 관측은 **처음부터 살아 있었고** 보고할
변화가 없었을 뿐입니다. 같은 시각 전체 기기:

| 기기 | 상태 | 알림 |
|---|---|---|
| 냉장고 3대 | 가동 중 | 7/9, 8/9, 8/9 → push |
| 에어컨 4대·후드·인덕션 | 꺼짐 | 0/27, 10/27, 14/27, 17/27, 1/9, 1/4 → polling |

**따라서 `OBSERVE_SUCCESS_FRACTION`은 유휴 가전에서 절대 통과할 수 없습니다.** 값을 0.8로
할지 더 낮출지 논쟁한 것은 잘못된 양을 두고 다툰 것입니다. 이 카운터로 내린 판단 중 최소
세 개가 틀렸습니다.

- **"등록마다 초기 2.05가 무조건 온다"** — 라이브러리의 의도이고 가전의 행동이 아닙니다.
- **"버스트가 초기 알림을 잃는다"** — 간격 0.05초를 넣은 뒤에도 꺼진 기기는 0/27입니다.
  잃을 초기 알림이 없습니다. 간격은 무해하고 라이브러리 자신의 `refresh_observes`와
  같으므로 남겨두지만, 무엇을 고쳤다고 주장하지 않습니다.
- **"꺼진 에어컨 4대가 27, 26, 3, 6이므로 활동량으로 설명 안 된다"** — 6시간 재구독
  주기의 서로 다른 지점을 비교한 것입니다. 높은 쪽은 하루치 누적, 낮은 쪽은 갓 재구독.

실질 피해는 작습니다. `_observing`이 바꾸는 것은 폴링 간격뿐(30초 ↔ 300초)이고, 틀리는
방향이 **안전한 쪽**입니다. 그래서 동작은 그대로 두었고, 고칠 것은 아래의 지표입니다 —
승격·강등 양쪽에 같은 결함이 있습니다.
...
**이 지표가 승격 쪽도 대체해야 합니다.** 위 실측이 보여주듯 `OBSERVE_SUCCESS_FRACTION`도
같은 오류를 가지고 있습니다 — 여기 적힌 "침묵은 고장이 아니다"를 강등에만 적용하고 승격에는
적용하지 않은 것이 원인입니다. 스윕 기반 누락 감지는 유휴 기기에도 성립하므로 양쪽을 한
번에 해결합니다. 그래서 이것이 미룰 항목이 아니라 **이 영역에서 유일하게 맞는 항목**입니다.
