...
### 변온고 모드 변경 — 냉장·냉동은 구현(1.0.1), 김치는 미해결

**1.0.1에서 냉장/냉동 전환을 구현했습니다.** capability를 `string`/읽기전용에서 `enum`/쓰기가능
으로 바꾸고 `_write_convertible_mode`를 추가했습니다. `x.com.samsung.da.modes`는 직교 플래그
여러 개를 한 리스트에 담으므로(두 대 모두
`["CVN_KIMCHI_STORAGE", "CV_NULL", "MULTIROOM_*"]`), `MULTIROOM_` 항목만 교체하고 나머지를
보존합니다 — 레퍼런스가 자기 기종의 같은 리소스에서 하는 방식과 같습니다.

**검증 수준을 정확히 적어 둡니다.** 두 대에 **현재 값을 그대로 다시 쓰는** 무해한 검증을
했고 리소스가 페이로드를 수락하고 플래그 목록이 온전히 유지됨을 확인했습니다. **다른 값으로
바꿨을 때 실제로 칸이 전환되는지는 확인하지 않았습니다** — 냉동실을 냉장으로 바꾸면 내용물이
녹으므로 소유자가 판단할 일입니다.

레퍼런스 이식을 실측이 막은 지점도 기록합니다. 레퍼런스의 `_flex_zone_current`는 "`modes`와
`supportedOptions`에 **동시에** 속한 항목"으로 찾는데, 우리 두 대의 `/mode/vs/0` 라이브 응답에는
**`supportedOptions` 필드가 아예 없습니다.** 그대로 이식하면 아무것도 바인딩되지 않습니다.
접두사 매칭을 유지한 것은 그 때문이고, 레퍼런스가 접두사 매칭을 버그로 고쳤다는 사실과
모순되지 않습니다 — 기종군이 다릅니다.

**김치는 여전히 막혀 있습니다.** 아래 항목 참고. 쓰기 토큰(`MULTIROOM_KIMCHI`?)도 미확인이고,
현재 강도를 읽을 필드도 없습니다. 미지 토큰은 이제 `None`으로 보고합니다 — enum이 거부할 값을
타일에 올리지 않기 위해서입니다.
