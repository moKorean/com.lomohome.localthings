...
**유력한 후보 두 가지. 둘 다 가설이고 확인되지 않았습니다.**

- **이전 앱 인스턴스의 고아 관측**. 재설치는 프로세스를 죽이므로 `close()`의 dereg가 돌지
  않고, 가전의 관측자 테이블에 죽은 등록이 남습니다. 연속 재설치는 0/27, 앱이 한동안
  없다가 설치하면 27/27이라는 관찰과 맞습니다. 이 앱은 기기별로 UDP 소스 포트를 고정하므로
  (RFC 6347 §4.2.8) 세션은 회수되지만 관측자 테이블은 별개일 수 있습니다.
- **등록 버스트**. 간격 0.05초를 넣기 전 진단이었고 폐기되지 않았습니다. 다만 이것만으로는
  같은 조건에서 0과 27이 갈리는 것을 설명하지 못합니다.

**확인 방법**: 재설치 직후와 앱을 10분 이상 내려둔 뒤 각각 초기 알림 수를 재고 비교합니다.
차이가 나면 고아 관측이고, 그렇다면 시작 시 `dereg` 후 재구독하거나 관측 토큰을 스토어에
남겨 다음 인스턴스가 정리하게 하는 것이 대책입니다.
