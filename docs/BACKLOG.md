...
`_push_is_healthy()`가 정의만 되어 있고 **어디에서도 호출되지 않습니다.** 그래서 한 번 push로
인정된 기기는 채널이 죽어도 최대 `OBSERVE_REFRESH_S`(6시간) 동안 그대로 유지되고, 그동안
5분 스윕에만 의존합니다.

그런데 **그 함수를 그대로 붙이면 안 됩니다.** 판정 기준이 "최근에 알림이 왔는가"인데, 꺼져 있고
조작이 없는 가전은 정상이어도 아무것도 보내지 않습니다. 실측: 에어컨 4대가 모두 꺼진 상태에서
알림 수가 27·26·3·6으로 갈렸는데, 이는 채널 상태가 아니라 **판정 시점에 기기가 돌고 있었는지**를
반영한 것이었습니다. 침묵을 고장으로 읽으면 유휴 기기를 계속 강등·재구독시킵니다.

필요한 신호는 다릅니다 — **5분 요약 스윕이 push가 놓친 변화를 발견했는가.** `_apply`가 이미
변화를 감지하므로, 관측 중인 href에서 스윕이 변화를 찾으면 그것이 실제 누락입니다. 침묵과 달리
유휴 상태를 오판하지 않습니다. 이 신호로 구현하는 것이 다음 작업입니다.
