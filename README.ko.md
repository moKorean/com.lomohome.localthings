...
| 건조기 (`DV*`) | 미검증. 동작상태·진행률·남은시간, 건조강도, 주름방지, 통세척까지 남은 횟수, **현재 코스 이름**(코스 표 2종, 건조 56종) |
| 식기세척기 (`ADW`, `DW*`) | 미검증. 동작상태·진행률, 살균, 가열건조, 누적수량, 사운드, 통세척까지 남은 횟수, **현재 코스 이름**(19종) |
| 공기청정기 (`AIR/TVTL/VTWW/AVT`) | 여기서는 미검증이지만 **블루스카이 5500이 동작한다는 사용자 제보가 있습니다.** 그 기기의 보드는 제보에 없어서 네 토큰 중 무엇으로 붙었는지는 모릅니다 — BACKLOG 참고. 풍량·표시등·펫필터·HEPA필터·공기질·PM10/2.5/1.0·주기적 공기질 감지와 최근 등급·감지 시 동작 |
