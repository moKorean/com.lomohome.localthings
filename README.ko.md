...
**운전 모드가 쾌적 모드보다 우선입니다.** 이 하드웨어에서 둘은 상호 배타이기 때문입니다 —
무풍을 걸면 AI 쾌적이 풀리고, AI 쾌적을 걸면 쾌적 모드가 `Off`로 떨어집니다. 처음 보고된
증상의 정체가 이것이었습니다: 플로우 마지막의 쾌적 모드 카드가 앞서 건 운전 모드를 되돌려서,
먼저 실행된 설정이 실패한 것처럼 보였습니다. AI 쾌적일 때는 요청하신 운전 모드를 지키기 위해
쾌적 모드를 건드리지 않습니다.
