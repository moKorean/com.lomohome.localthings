...
Homey는 **시스템 capability에만** 플로우 카드를 만들어 줍니다. 이 앱이 직접 정의한 86개는
카드가 없었고, 그래서 후드 조명을 플로우로 켜거나 필터 사용률로 알림을 보낼 수 없었습니다.

이제 **143장**입니다 — 동작 32, 조건 87, 트리거 24.
...
| 가전 | 상태 |
|---|---|
| **에어컨** (`RAC/PRAC/KRAC/CAC/WAC/FAC/CAWW/ARA`) | **실기기 검증.** 전원·희망온도·현재온도·모드·풍량·쾌적모드·풍향·공기청정·자동건조·표시등·엣지라이팅·UV살균·부재절전·습도·전력·먼지·먼지필터 2종(각각 세척 경보)·사운드·스마트 쿨 클린 상태/진행률·주기적 공기질 감지와 최근 등급·감지 시 동작 (48개) |
...
| **냉장고** (`REF`) | **실기기 검증** (TP2X_REF_21K 키친핏 3대 — 변온고 냉장/냉동, 일반). 칸별 현재온도와 목표온도(쓰기), 변온실 모드(읽기 전용 — 가전이 원격 변경을 받지 않습니다. 삼성 공식 앱에서도 되지 않습니다), 급속냉장(쓰기), 문열림, 누적전력 2종, 순간전력, 자체점검, 펌웨어 (9개) |
...
한국어와 영어를 지원하고, **선언되지 않은 언어는 영어로 표시됩니다** — 앱 이름·설명·capability 86개·플로우 카드 143장·설정 라벨·웹뷰 3개·기기 이름 전부. `tests/test_i18n.py`가 이를 강제합니다(한국어 문자열만 추가하고 영어를 빠뜨리는 실수는 작성자에게 보이지 않기 때문에).
