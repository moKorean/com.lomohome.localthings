...
Homey는 **시스템 capability에만** 플로우 카드를 만들어 줍니다. 이 앱이 직접 정의한 79개는
카드가 없었고, 그래서 후드 조명을 플로우로 켜거나 필터 사용률로 알림을 보낼 수 없었습니다.

이제 **134장**입니다 — 동작 30, 조건 80, 트리거 24.
...
| 청정스테이션 (`VSKR`, `VSWW`) | 실기기 미검증이지만 레퍼런스의 실기기 덤프로 읽기를 확인. 동작상태·먼지봉투 카운터·가득참 경고 |
...
한국어와 영어를 지원하고, **선언되지 않은 언어는 영어로 표시됩니다** — 앱 이름·설명·capability 79개·플로우 카드 134장·설정 라벨·웹뷰 3개·기기 이름 전부. `tests/test_i18n.py`가 이를 강제합니다(한국어 문자열만 추가하고 영어를 빠뜨리는 실수는 작성자에게 보이지 않기 때문에).
