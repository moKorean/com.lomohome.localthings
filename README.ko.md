...
| 세탁기 (`WW/WD/WF/WV/WA*`) | 미검증. 동작상태·진행률·남은시간, 세탁온도·탈수·헹굼, 누적수량, 통세척까지 남은 횟수, **현재 코스 이름**(코스 표 2종, 세탁 104종. 일체형 세탁건조기는 세탁·건조 코스를 각각 표시) |
| 건조기 (`DV*`) | 미검증. 동작상태·진행률·남은시간, 건조강도, 주름방지, 통세척까지 남은 횟수, **현재 코스 이름**(코스 표 2종, 건조 69종) |
| 식기세척기 (`ADW`, `DW*`) | 미검증. 동작상태·진행률, 살균, 가열건조, 누적수량, 사운드, 통세척까지 남은 횟수, **현재 코스 이름**(19종) |
...
| 에어드레서 (`DF`) | 실기기 미검증이지만 레퍼런스의 실기기 덤프 3종으로 읽기를 확인. 동작상태·진행률·남은시간·살균(쓰기)·주름방지(쓰기) |
