...
Homey는 **시스템 capability에만** 플로우 카드를 만들어 줍니다. 이 앱이 직접 정의한 100개는
카드가 없었고, 그래서 후드 조명을 플로우로 켜거나 필터 사용률로 알림을 보낼 수 없었습니다.

이제 **164장**입니다 — 동작 36, 조건 101, 트리거 27.
...
**이미 그 상태인 설정은 보내지 않습니다.** 설정 하나마다 쓰기·안정화 대기·되읽기가 드는데,
"집에 오면 냉방 24도, 풍량 3, 공기청정 켜기" 같은 플로우는 대개 이미 그 상태입니다. 건너뛰는
판단은 **직전에 기기에서 읽은 값**으로 합니다(카드 시작 시와 매 단계 후 다시 읽으므로 몇 초 이내의
값입니다) — 최대 5분 지날 수 있는 폴링 캐시가 아닙니다. 그리고 건너뛴 설정도 **마지막 검증에는
그대로 포함됩니다**: 한 설정을 적용하면 다른 설정이 밀릴 수 있고, 이미 맞아서 건너뛴 값이 바로
그 피해자가 될 수 있기 때문입니다. 보낼 것이 하나도 없으면 되읽기도 생략합니다.
...
가전이 광고하는 지원 값 목록에 없는 값은 보내기 전에 거부되는데, **그 거부는 목록을 함께
알려줍니다.** 둘은 구분할 값어치가 있습니다 — 가전이 받은 값을 거부한 것과 이 앱이 보내지
않기로 한 것은 다른 문제인데, 예전에는 문구가 똑같았습니다. 목록을 적어주는 것이 나중에 그
차이를 읽을 수 있게 하는 유일한 방법입니다. 거부된 쓰기는 아무것도 보내지 않으므로 살펴볼
통신이 없고, 관문이 무엇을 봤는지 남기는 곳도 달리 없습니다.
...
| 가전 | 상태 |
|---|---|
| **에어컨** (`RAC/PRAC/KRAC/CAC/WAC/FAC/CAWW/ARA`) | **실기기 검증.** 전원·희망온도·현재온도·모드·풍량·쾌적모드·풍향·공기청정·자동건조·표시등·엣지라이팅(밝기·색온도까지 쓰기 가능)·실외온도·UV살균·부재절전·습도·전력·먼지·먼지필터 2종(각각 세척 경보)·사운드·스마트 쿨 클린 상태/진행률·주기적 공기질 감지와 최근 등급·감지 시 동작·가전이 알람과 나란히 보고하는 운전 상태 코드 (52개) |
| **인덕션** (`COOKTOP`) | **실기기 검증.** 구별 화력·상태·잔열·냄비 감지·타이머, 전원, 차일드락, 스마트제어, 안전차단, 발생 알람, 전력, BT 온도 프로브 (26개). 전부 읽기 전용입니다 — 이 가전은 모든 쓰기에 4.05로 답합니다(실기기 확인) |
| **주방후드** (`AHD`) | **실기기 검증** (AHD-WW-TP1-22). 전원, 풍량 5단(쓰기), 조명 켜기/끄기와 밝기 2단(쓰기), 자동환기 상태, 필터 사용률·교체경보, 공기질·먼지 PM10/2.5/1.0, 주기적 공기질 감지와 최근 등급·감지 시 동작, 누적전력 (18개) |
| **냉장고** (`REF`) | **실기기 검증** (TP2X_REF_21K 키친핏 3대 — 변온고 냉장/냉동, 일반). 칸별 현재온도와 목표온도(쓰기), 변온실 모드(읽기 전용 — 가전이 원격 변경을 받지 않습니다. 삼성 공식 앱에서도 되지 않습니다), 급속냉장(쓰기), 문열림, 누적전력 2종, 순간전력, 자체점검, 펌웨어 (9개). **김치냉장고**도 여기로 붙고, 이제 칸별로 읽습니다 — 보관 모드(쓰기, 미검증), 숙성 상태와 남은 시간, 선반 수. 1도어와 3칸 배치 모두 |
| 세탁기 (`WW/WD/WF/WV/WA*`) | 미검증. 동작상태, **사이클 시작·일시정지·정지**, 진행률·남은시간, 세탁온도·탈수·헹굼, 누적수량, 통세척까지 남은 횟수, **현재 코스 이름**(코스 표 2종, 세탁 104종. 일체형 세탁건조기는 세탁·건조 코스를 각각 표시) |
| 건조기 (`DV*`) | 미검증. 동작상태, **사이클 시작·일시정지·정지**, 진행률·남은시간, 건조강도, 주름방지, 통세척까지 남은 횟수, **현재 코스 이름**(코스 표 2종, 건조 69종) |
| 식기세척기 (`ADW`, `DW*`) | 미검증. 동작상태, **사이클 시작·일시정지·정지**, 진행률, 살균, 가열건조, 누적수량, 사운드, 통세척까지 남은 횟수, **현재 코스 이름**(19종) |
| 공기청정기 (`AIR/TVTL/VTWW/AVT`) | 여기서는 미검증이지만 **블루스카이 5500이 동작한다는 사용자 제보가 있습니다.** 그 기기의 보드는 제보에 없어서 네 토큰 중 무엇으로 붙었는지는 모릅니다 — BACKLOG 참고. 풍량·표시등·펫필터·HEPA필터·공기질·PM10/2.5/1.0·CO2(보고하는 보드에서)·주기적 공기질 감지와 최근 등급·감지 시 동작 |
| 제습기 (`DHM`) | 미검증. 습도·목표습도(쓰기)·필터, 그리고 해당 리비전에서 표시등·물통 조명(둘 다 쓰기) |
| 오븐·레인지·전자레인지 (`OVEN/RANGE/MICROWAVE`) | 미검증. 동작상태, **동작 정지**(정지만 — 레퍼런스가 이 계열에는 시작·일시정지를 주지 않습니다), 모드·내부온도·설정온도·문열림 — **가열 제어 없음**. 가스 레인지는 **어느 화구가 켜져 있는지**도 보여줍니다(가스라고 선언한 보드에서만 해독). 최신 전자레인지 보드(`-0102X`)는 기존 계열의 리소스를 하나도 답하지 않아 자체 리소스로 읽습니다 — 문열림·차일드락·모드·조리시간·후드 기름필터 |
| 가스쿡탑 (`CT`) | 미검증. 전원 상태·버너 사용 여부 — **읽기 전용** |
| 정수기 (`WATERPURIFIER`) | 실기기 미검증이지만 레퍼런스의 실기기 덤프로 읽기를 확인. 동작상태·필터문 감지·잠금 3종(온수·냉수·버튼, 쓰기)·정수필터·누적수량 |
| 청정스테이션 (`VSKR`, `VSWW`) | 실기기 미검증이지만 레퍼런스의 실기기 덤프로 읽기를 확인. 동작상태·먼지봉투 카운터·가득참 경고·자동 비움·자동 닫힘(둘 다 쓰기) |
| 에어드레서 (`DF`) | 실기기 미검증이지만 레퍼런스의 실기기 덤프 3종으로 읽기를 확인. 동작상태, **사이클 시작·일시정지·정지**, 진행률·남은시간·살균(쓰기)·주름방지(쓰기) |
...
**사용자 제보는 검증과 다릅니다.** 공기청정기 행을 그렇게 적은 것은 의도적입니다. "실기기 검증"은 여기서 직접 페어링해 값을 읽었다는 뜻이고, 그래서 잘못된 매핑이 걸립니다. "동작한다"는 제보는 페어링이 되고 무언가 표시됐다는 뜻이라 유용한 신호이지만, **맞는 값과 그럴듯하게 틀린 값을 구별해 주지 못합니다** — 이 프로젝트가 이미 한 번 출시한 실패입니다(후드는 모든 필드가 틀린 채 페어링됐고 테스트는 통과했습니다). 이 행을 올려 줄 수 있는 것은 제보자의 `/device/0` 덤프입니다.
...
한국어와 영어를 지원하고, **선언되지 않은 언어는 영어로 표시됩니다** — 앱 이름·설명·capability 100개·플로우 카드 164장·설정 라벨·웹뷰 3개·기기 이름 전부. `tests/test_i18n.py`가 이를 강제합니다(한국어 문자열만 추가하고 영어를 빠뜨리는 실수는 작성자에게 보이지 않기 때문에).
