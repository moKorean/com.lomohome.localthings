...
| 가전 | 상태 |
|---|---|
| **에어컨** (`RAC/PRAC/KRAC/CAC/WAC/FAC/CAWW/ARA`) | **실기기 검증.** 전원·희망온도·현재온도·모드·풍량·쾌적모드·풍향·공기청정·자동건조·표시등·엣지라이팅(밝기·색온도까지 쓰기 가능)·UV살균·부재절전·습도·전력·먼지·먼지필터 2종(각각 세척 경보)·사운드·스마트 쿨 클린 상태/진행률·주기적 공기질 감지와 최근 등급·감지 시 동작·가전이 알람과 나란히 보고하는 운전 상태 코드 (51개) |
...
| 세탁기 (`WW/WD/WF/WV/WA*`) | 미검증. 동작상태·진행률·남은시간, 세탁온도·탈수·헹굼, 누적수량, **현재 코스 이름**(코스 표 2종, 세탁 82종. 일체형 세탁건조기는 세탁·건조 코스를 각각 표시) |
| 건조기 (`DV*`) | 미검증. 동작상태·진행률·남은시간, 건조강도, 주름방지, **현재 코스 이름**(코스 표 2종, 건조 38종) |
