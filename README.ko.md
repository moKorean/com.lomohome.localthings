...
이제 **144장**입니다 — 동작 32, 조건 88, 트리거 24.
...
| 오븐·레인지·전자레인지 (`OVEN/RANGE/MICROWAVE`) | 미검증. 동작상태, **동작 정지**(정지만 — 레퍼런스가 이 계열에는 시작·일시정지를 주지 않습니다), 모드·내부온도·설정온도·문열림 — **가열 제어 없음** |
...
한국어와 영어를 지원하고, **선언되지 않은 언어는 영어로 표시됩니다** — 앱 이름·설명·capability 87개·플로우 카드 144장·설정 라벨·웹뷰 3개·기기 이름 전부. `tests/test_i18n.py`가 이를 강제합니다(한국어 문자열만 추가하고 영어를 빠뜨리는 실수는 작성자에게 보이지 않기 때문에).
