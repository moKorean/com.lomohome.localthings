...
이제 **147장**입니다 — 동작 33, 조건 88, 트리거 26.
...
| **인덕션** (`COOKTOP`) | **실기기 검증.** 구별 화력·상태·잔열·냄비 감지·타이머, 차일드락(쓰기), 전원(끄기만 — 가전이 원격 켜기를 받지 않습니다), 스마트제어, 안전차단, 발생 알람, 전력, BT 온도 프로브 (26개) |
...
한국어와 영어를 지원하고, **선언되지 않은 언어는 영어로 표시됩니다** — 앱 이름·설명·capability 87개·플로우 카드 147장·설정 라벨·웹뷰 3개·기기 이름 전부. `tests/test_i18n.py`가 이를 강제합니다(한국어 문자열만 추가하고 영어를 빠뜨리는 실수는 작성자에게 보이지 않기 때문에).
